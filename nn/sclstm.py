...
class sclstm(BaseRLG):
    ...
    def _init_params(self):
        ...
        self.Wgate = theano.shared(0.3 * np.random.uniform(-1.0, 1.0,
                                                           (self.dh * 2 + self.dsv, self.dh * 3)).astype(
            theano.config.floatX))
        # for reading gate
        self.Wrgate = theano.shared(0.3 * np.random.uniform(-1.0, 1.0,
                                                            (self.dh * 2 + self.dsv, self.dsv)).astype(
            theano.config.floatX))
    ...
    def unroll(self, a, sv, words, cutoff_f, cutoff_b):
        ...
        semcost = T.sum(abs(f[0, :, :] - sv_1hot)) + T.sum(abs(f[-1, :, :])) + T.sum(0.0001 * (100.0 ** abs(f[:-1, :, :] - f[1:, :, :])))
    ...
    def _recur(self, w_t, y_t, sv_tm1, h_tm1, c_tm1, a):
        ...
        # compute reading rg
        rg = T.nnet.sigmoid(T.dot(T.concatenate([wv_t, h_tm1, sv_tm1], axis=1), self.Wrgate))
        ...
        c_t = ig * cx_t + fg * c_tm1 + T.tanh(T.dot(T.concatenate([a, sv_t], axis=1), self.Wfc))
